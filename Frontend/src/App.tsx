import React from 'react';
import { BrowserRouter, Routes, Route, Navigate } from 'react-router-dom';
import { Layout } from './components/layout/Layout';
import { Dashboard } from './pages/Dashboard';
import { LiveCallMonitorPage } from './pages/LiveCallMonitorPage';
import { ActiveCallsPage } from './pages/ActiveCallsPage';
import { CallHistoryPage } from './pages/CallHistoryPage';
import { AnalysisReportsPage } from './pages/AnalysisReportsPage';
import { EvidenceViewerPage } from './pages/EvidenceViewerPage';
import { SettingsPage } from './pages/SettingsPage';
import { HelpPage } from './pages/HelpPage';
import { DeveloperPage } from './pages/DeveloperPage';

import { ErrorBoundary } from './components/common/ErrorBoundary';


export const App: React.FC = () => {
  return (
    <BrowserRouter>
      <ErrorBoundary>
        <Routes>
          <Route path="/" element={<Layout />}>
            <Route index element={<Dashboard />} />
            <Route path="active-calls" element={<ActiveCallsPage />} />
            <Route path="live-monitor" element={<LiveCallMonitorPage />} />
            <Route path="live-monitor/:callId" element={<LiveCallMonitorPage />} />
            <Route path="call-history" element={<CallHistoryPage />} />
            <Route path="reports" element={<AnalysisReportsPage />} />
            <Route path="reports/:callId" element={<AnalysisReportsPage />} />
            <Route path="evidence" element={<EvidenceViewerPage />} />
            <Route path="evidence/:callId" element={<EvidenceViewerPage />} />
            <Route path="settings" element={<SettingsPage />} />
            <Route path="help" element={<HelpPage />} />
            <Route path="*" element={<Navigate to="/" replace />} />
          </Route>
          {/* Hidden developer route — NOT in sidebar, access via /developer directly */}
          <Route path="/developer" element={<DeveloperPage />} />
        </Routes>
      </ErrorBoundary>
    </BrowserRouter>
  );
};

export default App;
